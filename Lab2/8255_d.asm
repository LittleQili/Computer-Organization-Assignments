...
L1: 
		IN AL,PortC			;READ
		MOV AH,AL			;store the value of input
		
		;LED display 1&3
		AND AL,0F0H			;get the higher 4-bit of input(PC4~PC7)
		MOV DL,AL			;store the value of PC4~PC7
		ADD AL,0AH			;enlight LED display 1&3
		OUT PortA,AL
		
		MOV AL,AH
		MOV CL,4
		SHR AL,CL			;get PC4~PC7 and put them into lower bit of a byte
		MOV BX,OFFSET SEGTAB
		ADD BL,AL
		MOV AL,[BX]			;calculate the addr of LED display input data
		OUT PortB,AL
		
		CALL DELAYS			;DelaySShort, display result correctly

		;LED display 2&4
		MOV AL,DL
		ADD AL,5H			;enlight LED display 2&4
		OUT PortA,AL
						;below are the same as task c
		MOV AL,AH
		AND AL,0FH
		MOV BX,OFFSET SEGTAB
		ADD BL,AL
